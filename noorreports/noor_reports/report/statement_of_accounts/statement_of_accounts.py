...
class ReceivablePayableReport(object):
    ...
    def build_data(self):
        ...
        for key, row in self.voucher_balance.items():
            row.outstanding = flt(row.invoiced - row.paid - row.credit_note + amt, self.currency_precision)
            amt = row.outstanding
    ...
    def get_gl_entries(self):
        ...
        self.gl_entries = frappe.db.sql("""
			...
			where
				...
				{1} {2} {3}"""
                                        .format(select_fields, date_condition, conditions, order_by), values,
                                        as_dict=True)
    ...
    def get_cost_center_conditions(self, conditions):
        ...
        cost_center_list = [center.name for center in
                            frappe.get_list("Cost Center", filters={'lft': (">=", lft), 'rgt': ("<=", rgt)})]
    ...
    def setup_ageing_columns(self):
        ...
        for i, label in enumerate(["0-{range1}".format(range1=self.filters["range1"]),
                                   ...
                                   "{range3}-{range4}".format(range3=cint(self.filters["range3"]) + 1,
                                                              range4=self.filters["range4"]),
                                   "{range4}-{above}".format(range4=cint(self.filters["range4"]) + 1,
                                                             above=_("Above"))]):
